...
file_garmin = "2023_04_16_16_25_41.json"
file_p2max = "2023_04_15_11_29_13.json"
```
...
#data_garmin %>% 
  write_csv2("Results/power_meter_comparison/2023_04_16_p2max_crosser_element_kalib_data.csv")
data_p2max <- get_activity_data(data_p2max)
#data_p2max %>% write_csv2("Results/power_meter_comparison/2023_04_15_p2max_data.csv.csv")
```
...
file_garmin <- "2023_04_16_garmin_crosser_uhr_kalib_data.csv"
file_p2max <- "2023_04_16_p2max_crosser_uhr_kalib_data.csv"
...
data_garmin %>% mutate(SECS = SECS-6 ,
                        WATTS = WATTS -12) %>% filter( SECS > secs_min &
                          SECS < secs_max) %>%
  ggplot(aes(x= SECS)) + 
  geom_line(aes(y= WATTS, color = "garmin")) +
...
data_garmin %>% mutate(SECS = SECS+2) %>%
  ggplot(aes(x= SECS)) + 
...
data_garmin %>% mutate(SECS = SECS-5) %>%
  ggplot(aes(x= SECS)) + 
  geom_line(aes(y= HR, color = "garmin")) +
  geom_line(data = data_p2max,aes(y= HR, color = "p2max"))+
...
```{r}
data_p2max_garmin <- data_garmin %>% mutate(SECS = SECS+2) %>% 
...
```{r}
data_p2max_garmin %>% write_csv2("Results/power_meter_comparison/2023_04_16_powermeter_p2max_crosser_vs_garmin_uhr_kalib.csv")
```
