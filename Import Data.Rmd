...
if((files_selected$file_name[i] %in% old_aggregations$file) | update == TRUE){
  print("yes")
}else{print(files_selected$file_name[i])}
}
```

Hier weitertmachen:
-fehler unten ansehen und schritt für schritt ausbessern
(in eigener comand cell mit immer nur einer datei)
## Aggregation
```{r}
update = FALSE
    ...
    datei <- str_c(pfad_aktivity,files_selected$file_name[i])
      try({
           ...
           if (samples == 1) {  #there is trainings data
                  ...
                  aggregation_ride$cadence <-ridedata%>% summarise(cadence = as.integer(mean(CAD,na.rm = TRUE))) %>% pull()
