...
```{r}
selected_files_er10 <- result_quality %>% filter(pred_error >10)
selected_files_er10
```
...
```{r}
selected_files_ga1 <- result_quality %>% filter(pred_error >5 & 
...
### Predict
```{r}
result_xgb <- data_model %>%
                  bind_cols(
                    predict(final_fit_xgb, new_data = data_model) %>% rename("hr_pred" = ".pred")
                  ) %>% mutate(set = "test")
```
...
```{r}
selected_files <- result_quality %>% filter(pred_error >10)
selected_files
```
...
```{r}
result_xgb %>% filter(file_name %in% selected_files_ga1$file_name[1:3]) %>%
  ggplot(aes(x=minute))+
  geom_line(aes(y=hr, color = "hr"))+
...
result_all %>% filter(file_name %in% selected_files_er10$file_name[]) %>%
  ...
  #geom_line(data = result_final_xgb %>% filter(file_name %in% files_test),
   #         aes(y=hr_pred, color ="prediction old"))+
  #geom_line(aes(y=watt, color ="watt"), alpha =0.5)+
  facet_wrap(workout_code ~ ., nrow = 4)+
  labs(title = "prediction of never seen  test data",
       colour = "")
```
