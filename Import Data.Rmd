...
calculate_TSS <- function(d,FTP){
    ...
    return(as.numeric(NA))
      ...
      mutate(intensity = watt_np/FTP,
             TSS = (sek* watt_np * intensity)/(FTP * 3600)*100)
      return(d$TSS)
      }
...
for (i in seq_along(files_selected$file_name)){ 
  ...
  try({
     ...
     aggregation_ride <- tibble("workout_code" = as.character(data_ride$RIDE$TAGS$`Workout Code`))
            ...
            if (samples == 1) {  #there is trainings data
                ...
                override <- data_ride$RIDE$OVERRIDES
                    ...
                    for (i in seq_along(override)){
                      override_name <- data_ride$RIDE$OVERRIDES[[i]] %>% names()
                      override_value <- data_ride$RIDE$OVERRIDES[[i]][[1]]$value
                      if(override_name == "elevation_gain"){
                        aggregations$elevation_gain <- override_value
                      }else if(override_name == "coggan_tss"){
                        aggregations$TSS <- override_value
                      }
                    }
 ...
 aggregation_ride  #%>% str()
 
 bind_rows(aggregation_rides) %>% mutate(start_time = as.POSIXct(start_time)) 
 ##HIER STIMMT WAS NICHT SOLLTEN 3 ERGEBNISSE SEIN
 #aggregation_rides[[2]]
```
