...
data_model <-  data_2017_minute %>% #select(hr,watt, file_name)
                        ...
                        arrange(minute) %>%
                            mutate( hr_3 = lag(hr,3),
                                   hr_5 = lag(hr,5),
                                   hr_10 = lag(hr,10)) %>%
  # calculate the mean watt of the last x minutes
...
data_model %>% select(watt, watt_m3,everything())
```
...
rmse_files <- result_final %>% group_by(file_name) %>% rmse( hr,hr_pred) %>% arrange(desc(.estimate))

rmse_files %>% ggplot()+geom_histogram(aes(.estimate), binwidth =1)+
  labs(x = "RMSE")+
  theme_light()
```

2017_01_09_19_31_11.json:

wollte 50 Minuten SST fahren, leider bin ich anscheinend doch noch zu krank
mein Puls ist auf 170 rauf (25min) habe dann 5min pause gemacht, damit der puls wieder runter kommt - von den beinen her wäre es locker gegangen.
nach 10min war er wieder oben, haeb die 40min voll gemacht und bin ausgefahren. hoffentlich bin ich morgen nicht krank sonder fitt...


```{r}

files_test <- rmse_files$file_name[1:6]
...
rmse_files_xgb <- result_final_xgb2 %>% group_by(file_name) %>% rmse( hr,hr_pred) %>% arrange(desc(.estimate))

rmse_files_xgb %>% 
  ggplot()+
  geom_histogram(aes(.estimate, fill = "XGB"), binwidth =0.5, alpha =1/2)+
  geom_histogram(data = rmse_files,
                 aes(.estimate, fill = "RF beginning"), binwidth =1, alpha =1/2)+
  labs(x = "RMSE")+
  theme_light()
```
...
### Check OVERFITTING
```{r}
train_data2 %>% filter(str_detect(workout_code, "FTP Test Zwift")) %>% count(file_name)
```
...
result_final_xgb2_2 %>% filter(file_name %in% files_test) %>%
  ...
  #geom_line(aes(y=watt, color ="watt"), alpha =0.5)+
  facet_wrap(workout_code ~ ., nrow = 3, scales = "free")+
...
```{r}
files_test <- c("2017_01_02_15_20_44.json" )

result_final_xgb2_2 %>% filter(file_name %in% files_test) %>%
  
  ggplot(aes(x=minute))+
  geom_line(aes(y=hr, color = "hr"), size =1)+
  geom_line(aes(y=hr_pred, color = "hr_pred"), size =1)+
  geom_line(aes(y=watt, color = "watt"), size =1)+
  #geom_line(aes(y=watt, color ="watt"), alpha =0.5)+
  facet_wrap(workout_code ~ ., nrow = 3, scales = "free")+
  labs(title = "prediction error of never seen  FTP Test Zwift",
       colour = "",
