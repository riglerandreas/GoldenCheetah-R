...
source("functions_plot_seasons.R")
source("functions_ftp_laktat.R")
```
...
plot_ftp_progress_bar_chart(ftp_values)
```
...
letzte tag sollte der nächste ftp tag sein (dann ist die ftp erreicht) - oder ein tag davor,
...
data_gc %>% summarise( date_min =min(date),
                       date_max = max(date))
```

```{r}
data_gc %>% filter(L_R >0 &
                     date > as.Date("2018-01-01")) %>%
  ggplot(aes(x=date))+
  geom_point(aes(y=L_R))+
...
```{r}
rename_difficult <- function(df, col_contains_1, col_contains_2 ="", new_name){
  #function to rename a column in the df with a complex name (UTF..)
  #col_contains_1  & _2 are key words which are used to search the right column to rename
  #
  
  col_names_old <- df %>% colnames()
  
  col_to_rename <- tibble(colnames = col_names_old) %>% 
                              filter(str_detect(colnames, col_contains_1)&
                                      str_detect(colnames, col_contains_2)) %>%
                                pull()
  cols_found <- col_to_rename %>% length()
  
  if (cols_found == 1 ) {
    
    df_renamed <- df %>% rename(!!sym(new_name) := col_to_rename)

    return(df_renamed)
    
  }else{
    print(str_c(cols_found, " found with ",col_contains , "!"))
  }
...
rename_difficult(data_gc, "L_R","",new_name = "5") %>% colnames()

```


```{r}
data_gc %>% filter(Drehmoment_Effektivitaet_Rechts >0 &
                     date > as.Date("2018-01-01")
                   ) %>%
  ggplot(aes(x=date))+
  geom_point(aes(y=Drehmoment_Effektivitaet_Rechts, color = "rechts"))+
  geom_point(aes(y=Drehmoment_Effektivitaet_Links, color = "links"))+
  geom_vline( aes(xintercept = as.Date("2023-06-13"),color = "Thrombose"))+
  geom_vline( aes(xintercept = as.Date("2023-09-25"),color = "Intervention"))+
...
data_gc %>% filter(Drehmoment_Effektivitaet_Rechts >0 &
  ...
  mutate(label = case_when(date >= as.Date("2023-09-25") ~ "Intervention")) %>%
  ggplot(aes(x=Drehmoment_Effektivitaet_Rechts))+
  geom_point(aes(y=Drehmoment_Effektivitaet_Links, color = label))+
...
#add today to the plot data:
min_last <- plot_data_intervall_time %>% ungroup() %>% 
                                filter(date == max(date)) %>% select(min) %>% pull()

data_last <- tibble(date = Sys.Date(),
       min = min_last,
       year = as.factor(year(date)),
       day = yday(date))

plot_data_intervall_time <- plot_data_intervall_time %>% bind_rows(data_last)

# plot
plot_data_intervall_time %>%
  filter(day < 350 & year %in% c(2016,2017,2018,2021, 2022,2023)) %>%
...
plot_data_gc %>%
  filter(day < 360 & year %in% c(2016,2017,2020,2021, 2022,2023)) %>%
...
```{r}
seasons <- c(2016,2017,2020,2021, 2022,2023)



plot_km_seasons(data_gc, seasons, days_plot = 365, km_goal = 9000)
```
...
plot_hm_seasons(data_gc, seasons, days_plot = 365, km_goal = 9000)
```

```{r}

plot_hm_km(data_gc, seasons)
...
4 Wochen passt nicht 2021-01-28 interval_watt 249W !! 100 min für 4 wochen
...
##### Einschub ML
Model für jede Kurve trainieren und FTP predicten
