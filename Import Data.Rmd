...
files_selected %>% filter(str_detect(file_name,"24_14"))
...
```{r}
laktat_watt_raw <- read_csv2(str_c(path_ftp,"laktat_watt.csv"))

laktat_watt <- laktat_watt_raw %>% mutate(date = as.Date(datum, "%d.%m.%Y")) %>% select(-datum) %>% arrange(date)%>% mutate(date_max = lead(date) - 1,
                    date_min = date,
                    date_max = if_else(is.na(date_max),Sys.Date(),date_max))
...
# Hier weitermachen:
  - 2mmol and 4mmol value
- calculate time < 2mmol watt
- calculate time > 4mmol
...
override = FALSE  #If override = TRUE .. old aggregations will be overwritten, override = FALSE.. new date will be appended

if( override == FALSE){
  
 old_aggregations <- read_rds(str_c(path_laktat_ftp,"Results/aggregations_result.rds"))
}
...
for (i in seq_along(files_selected$file_name)){
  if((!(files_selected$file_name[i] %in% old_aggregations$file_name)) | update == TRUE){
...
if(!is_empty(aggregation_rides)){
  ...
  #Save the result:
  write_rds(aggregations_result, str_c(path_laktat_ftp,"Results/aggregations_result.rds"))
}
